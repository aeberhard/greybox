 ...
 */
	if ( isset($_POST['function']) and ($_POST['function']=='save') ) {
		...
		if (isset($_POST['footerout'])) {
			$footerout = $_POST['footerout'];
		} else {
			$footerout = '';
		}
		if (trim($footerout=='')) $footerout = 0;
		
		$line = $allcats.','.$subcats.','.$footerout."\n";
		...
		if (($fh = fopen($rxa_greybox['path'].'/'.$rxa_greybox['name'].'.ini', 'w')) === FALSE) {
			$rxa_greybox['meldung'] = $rxa_greybox['i18n']->msg('error_save',$rxa_greybox['path'].'/'.$rxa_greybox['name'].'.ini');
		} else {
			@fwrite($fh, $line);
			@fclose($fh);	
			$rxa_greybox['meldung'] = $rxa_greybox['i18n']->msg('msg_saved');
		}
	}
 ...
 */
	if (($lines = file($rxa_greybox['path'].'/'.$rxa_greybox['name'].'.ini')) === FALSE) {
		$rxa_greybox['meldung'] = $rxa_greybox['i18n']->msg('error_read',$rxa_greybox['path'].'/'.$rxa_greybox['name'].'.ini');
	} else {
		...
		$rxa_greybox['cats'] = unserialize(trim($lines[1]));
		$rxa_greybox['excludeids'] = unserialize(trim($lines[2]));
	}
	if (!is_array($rxa_greybox['cats'])){
		$rxa_greybox['cats'] = array();
	}
 ...
 */
	...
	if ($cats = OOCategory::getRootCategories())
	{
		foreach( $cats as $cat)
		{
			add_cat_options($rxa_greybox, $select_cats, $cat, $cat_ids);
		}
	}
	...
	add_rootart_options($rxa_greybox, $select_cats, $clang);

	$selsize = $rxa_greybox['catselectcount'] / 3;
...
<?php
	if ($rxa_greybox['meldung']<>'') echo rex_info($rxa_greybox['meldung']);
?>

<?php echo $rxa_compat['backendprefix']; ?>

<div class="rex-addon-output">
<h2 class="rex-hl2"><?php echo $rxa_greybox['i18n']->msg('menu_settings'); ?></h2>

<div class="rex-form">

<form action="index.php?page=<?php echo $rxa_greybox['name']; ?>" method="post">
...
<input type="hidden" name="function" value="save" />
	...
	<div class="rex-addon-content">
		<p>
<?php
	echo $rxa_greybox['i18n']->msg('text_settings_intro');
?>
		</p>
	</div>
	</div>		
	...
	<div class="rex-form-row">
		<p class="rex-form-col-a rex-form-checkbox rex-form-label-right">
			...
			<label for="allcats"><?php echo $rxa_greybox['i18n']->msg('text_settings_allcats'); ?></label>
		</p>
	</div>
	...
	<div class="rex-form-row">
		<p class="rex-form-col-a rex-form-checkbox rex-form-label-right">
			...
			<label for="subcats"><?php echo $rxa_greybox['i18n']->msg('text_settings_subcats'); ?></label>
		</p>
	</div>

	<div class="rex-form-row">
        <p class="rex-form-col-a rex-form-select">
			<label for="id_category_select"><?php echo $rxa_greybox['i18n']->msg('text_settings_help'); ?></label>
<?php
	echo $select_cats;
?>
		</p>
	...
	<div class="rex-form-row">
		<p class="rex-form-col-a rex-form-text">
			<label for="excludeids"><?php echo $rxa_greybox['i18n']->msg('text_settings_excludeids'); ?></label>
			<input class="rex-form-text" type="text" id="excludeids" name="excludeids" value="<?php echo $rxa_greybox['excludeids']; ?>" />
			<span class="rex-form-notice"><?php echo $rxa_greybox['i18n']->msg('text_settings_excludeids2'); ?></span>
		</p>
	</div>

	<div class="rex-form-row">
		<p class="rex-form-col-a rex-form-checkbox rex-form-label-right">
			<input class="rex-form-checkbox" type="checkbox" id="footerout" name="footerout" value="1" <?php if ($footerout == "1") echo "checked"; ?> />
			<label for="footerout"><?php echo $rxa_greybox['i18n']->msg('text_settings_footer'); ?></label>
		</p>
	</div>
	
	<div class="rex-form-row">
		<p class="rex-form-col-a rex-form-submit">
			<br /><input type="submit" class="rex-form-submit" name="sendit" value="<?php echo $rxa_greybox['i18n']->msg('button_save'); ?>" /><br /><br />
		</p>
	</div>
